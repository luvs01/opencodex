...
只在首次註冊供應商時套用；更新、重新登入與更換金鑰不會重設既有選擇。初始化後，可在 Models 或使用以下 CLI 指令啟用所需模型。後續新增模型的獨立政策不變。
...
個別模型的切換請參閱 `ocx models --help`。標記為 `native` 的列需要加 `--native`；包含 `/` 的 ID 必須使用，否則斜線會被解析為 `provider/model`。模型 ID 屬於不可信的上游資料，切勿將其貼到 shell 指令字串中。
