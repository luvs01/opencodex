...
describe("planInteractionQueryReply", () => {
  ...
  // Cursor-native web/exa gates can spend quota and send model-selected data to external services.
  // Without explicit request-level authorization the headless bridge must reject them.
  test.each([
    ...
    const value = plan.response.result.value as { result: { case: string } };
    expect(value.result.case).toBe("rejected");
    expect(plan.replyCase).toBe(`${responseCase}:rejected`);
  });
