...
明示的なバリアントは、Cursor の `default` モデルを `optimization` パラメータとともに送信し、リクエストごとに選択を保持します。ライブディスカバリーが成功した場合、アカウント固有の応答で許可された明示的なバリアントのみが表示され、`cursor/auto` は安全なフォールバックとして残ります。
