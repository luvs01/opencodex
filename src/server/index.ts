...
export function startServer(port?: number, deps: StartServerDeps = {}): Server<WsData> {
  ...
  // The integration switch controls Codex config/catalog synchronization, not
  // data-plane account routing. Retain native-main ownership even while that
  // integration is disabled so recovery state still fences auth.json reads.
  const nativeOwnership = inspectNativeCodexOwnership();
  const nativeMainLifecycle: NativeMainStartupLifecycle = nativeOwnership.ownership !== "foreign"
