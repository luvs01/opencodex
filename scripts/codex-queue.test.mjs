...
// on Windows. Only actual queue submissions are logged; --help is read-only.
...
const fields = [process.env.CODEX_HOME || '', process.argv[1], ...args];
...
class QueueStub {
  static int Main(string[] args) {
    ...
    string[] fields = new string[] { Environment.GetEnvironmentVariable("CODEX_HOME") ?? "", exe }.Concat(args).ToArray();
...
function fixture() {
  ...
  const env = { ...process.env, HOME: home, USERPROFILE: home, CODEX_HOME: store,
    LOCALAPPDATA: join(root, 'local app data'), STUB_LOG: log,
  ...
  return { root, home, store, exe, log, env };
}
...
function records(f) {
  ...
  return readFileSync(f.log, 'utf8').trim().split('\n').map(line => {
    const [home, exe, ...args] = line.split('|').map(field => Buffer.from(field, 'base64').toString());
    return { home, exe, args };
  });
}
...
for (const shell of shells) {
  ...
  describe(`${shell} native queue helper`, { skip: !available }, () => {
    function run(f, options = {}) {
      ...
      const flag = (name, value) => { flagPositions.add(args.length); args.push(ps ? '-' + name : '--' + ({ CodexExe: 'codex', DryRun: 'dry-run' }[name] || name.toLowerCase())); if (value !== undefined) args.push(value); };
      ...
      const command = '& ' + args.map((arg, index) =>
    ...
    if (windows) {
      ...
      it('discovers the Windows app bundle before standalone/PATH (dry run only)', () => {
        ...
        assert.equal(result.status, 0, result.output); assert.ok(result.output.includes(bundled));
        assert.deepEqual(records(f), []);
      });
    }
    if (!ps) {
      ...
      it('finds the standalone bin layout (discovery only)', { skip: existsSync('/Applications/Codex.app/Contents/Resources/codex') }, () => {
        ...
        const result = run(f, { pin: false, thread: threadA, dryRun: true });
        assert.equal(result.status, 0, result.output); assert.ok(result.output.includes(standalone));
        assert.deepEqual(records(f), []);
      });
    }
  });
