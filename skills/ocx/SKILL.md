...
Hub invites have the same boundary: both text and JSON output contain a plaintext
pairing grant and a command embedding it. Ask the user to run the invite in a
human-operated terminal outside the agent session and report only non-secret
confirmation, such as the expiry and bound browser origin. Never ask them to paste
the grant or generated command into chat.
...
**A hub is one port, and `ocx hub invite` writes the join command for the operator.** Remote machines dial
...
(`unauthenticatedLoopbackListener: {"enabled": true}`, no port). Ask the user to run `ocx hub
invite` on the hub in a human-operated terminal outside the agent session rather than assembling
an `ocx connect` line: it mints a single-use code and prints the exact command, with both origins
already filled in. Its `--management-url` is a confirmation of `hub.managementPublicOrigin`, not
an override. The user must transfer that command directly to the other machine without exposing it
