...
처음 등록할 때만 적용하며 업데이트, 재로그인, 키 교체로 기존 선택을 초기화하지 않습니다. 초기 설정이 끝나면 Models 탭이나 아래 CLI 명령으로 필요한 모델을 켤 수 있습니다. 이후 새 모델이 추가될 때의 정책은 별도입니다.
...
개별 모델 전환은 `ocx models --help`를 참조하세요. `native`로 표시된 행에는 `--native`가 필요하며 `/`가 포함된 ID에서는 필수입니다. 그렇지 않으면 슬래시가 `provider/model`로 해석됩니다. 모델 ID는 신뢰할 수 없는 업스트림 데이터이므로 셸 명령 문자열에 붙여 넣지 마세요.
