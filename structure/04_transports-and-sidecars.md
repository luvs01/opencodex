...
Synthesized output is capped at 10,000 items and HTTP frames are encoded incrementally, so the
bounded upstream JSON cannot expand into an unbounded in-memory event array or SSE string.
