...
| Providers | Create/update/delete ordinary provider configs and enrich registry metadata. Every completed POST candidate passes the full persisted-config validator before live adoption; relative Responses and Chat send-path overrides are also checked at the management boundary. The reserved `openai` card exposes Pool(default)/Direct account mode; `openai-apikey` remains the separate API route. |
