...
trames WS, même lorsqu’un alias sélectionne Spark — uniquement si le corps sortant ne porte aucun
groupe `additional_tools` non vide. Un groupe avec `tools: []` n’active pas Lite. Un groupe non vide
EST la forme Lite de livraison des outils : un corps Spark
