...
выборе Spark через псевдоним — но только если в исходящем теле нет непустой группы `additional_tools`.
Группа с `tools: []` не включает Lite. Непустая группа и ЕСТЬ Lite-форма доставки инструментов,
поэтому тело Spark, которое её использует,
