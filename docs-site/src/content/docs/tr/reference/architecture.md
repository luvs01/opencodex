...
seçildiğinde de bu geçerlidir — ancak yalnızca giden gövde boş olmayan bir `additional_tools` grubu
taşımıyorsa. `tools: []` içeren bir grup Lite'ı etkinleştirmez. Boş olmayan grup Lite'ın araç teslim
biçiminin kendisidir; onu kullanan bir Spark gövdesi,
