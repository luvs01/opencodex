...
선택해도 동일합니다. 다만 이 비활성화는 전송 본문에 비어 있지 않은 `additional_tools` 그룹이 없을 때만
적용됩니다. `tools: []`인 그룹은 Lite를 켜지 않습니다. 비어 있지 않은 그룹 자체가 Lite의
도구 전달 형식이므로, 그것을 사용하는 Spark 본문은 호출자나
