...
describe("systemd service unit", () => {
  test("does not select the systemd executable from PATH", () => {
    const inheritedPath = process.env.PATH;
    try {
      process.env.PATH = "/tmp/untrusted-bin:/usr/bin:/bin";
      const unit = buildUnit(resolvedProxyEnv({}));

      expect(unit).not.toContain("/tmp/untrusted-bin/ocx");
      expectTextToContainPath(unit, join("cli", "index.ts"));
      expect(unit).toContain("OCX_BUN_RUNTIME_PATH");
    } finally {
